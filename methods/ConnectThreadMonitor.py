...
from crypto import algo, get_decrypt
...
class message_monitor(QtCore.QThread):
        ...
        while True:
            if self.server_socket != None:
                ...
                # ['ENCRYPT_MESSAGE', self.nick, smile_num, message_text.encode()]
                    ...
                    decrypted_text = get_decrypt(self.config, text=decrypted_text)
                    logger.info(f"Кординаты расифрованы: {decrypted_text}")
    ...
    def send_encrypt(self, data_list):
        ...
        if data_list[0] == "ENCRYPT_MESSAGE":
            # ! Смена шифра
            data_list[-1] = algo(config=self.config, text=data_list[-1].decode())
            logger.info(f"Кординаты зашифрованы: {data_list[-1]}")
            encrypted_message = self.cipher.encrypt(data_list[-1].encode())
